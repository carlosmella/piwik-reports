...
namespace Piwik\Plugins\ReportesPersonalizados\Reports;
 ...
 */
class GetUrlstiemporeal extends Base
{
    protected function init()
    {
        parent::init();

        $this->name          = Piwik::translate('PluginPrueba_Urlstiemporeal');
        $this->dimension     = null;
        $this->documentation = Piwik::translate('Obtener las urls en las que estan los usuarios en tiempo real');
        ...
        // If a menu title is specified, the report will be displayed in the menu
         $this->menuTitle    = 'Urls tiempo real';
        ...
        $this->widgetTitle  = 'URLs Tiempo Real';
    }
    ...
    public function configureView(ViewDataTable $view)
    {
       // if (!empty($this->dimension)) {
            $view->config->addTranslations('ip','Direccion IP');
       // }
	...
	//$view->config->show_visualization_only = 'table';	

	$view->config->show_table_all_colums = false;
        $view->config->columns_to_display = array_merge(array('ip','url','date'));
    }
    ...
    {
        $view = new View('@PluginPrueba/getUrlstiemporeal');
        $view->myData = array();
